...
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1045_sm::Kernel_TopKernel_CounterChain1045_sm(maxcompilersim::DebugStreams *debug_output, int node_number) :
...
void
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1045_sm::reset() {
	state2 = 0;
	state3 = 0;
	state4 = 1;
	state1 = 0;
...
void
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1045_sm::execute(const bool sm_not_flushing) {
	...
	if (inputdata_reset) {
		...
		switch (static_cast<int>(state1)) {
			case 0:
				if (inputdata_en) {
					if (varint_u<1>((state2 + varint_s<32>(0x0l)) >= (inputdata_max0 - varint_s<32>(0x60l))) & varint_u<1>((state3 + varint_s<32>(0x0l)) >= (inputdata_max1 - varint_s<32>(0x60l)))) {
						copy_state1 = varint_u<1>(0x1l);
					} else {
						if (varint_u<1>((state3 + varint_s<32>(0x0l)) >= (inputdata_max1 - varint_s<32>(0x60l)))) {
							copy_state3 = varint_s<32>(0x0l);
							if (varint_u<1>((state2 + varint_s<32>(0x0l)) == (state4 - varint_s<32>(0x60l)))) {
								copy_state2 = (state2 + varint_s<32>(0x60l)) + varint_s<32>(0x0l);
								copy_state4 = (state4 + varint_s<32>(0x0l)) + varint_s<32>(0x1l);
							} else {
								copy_state2 = state2 + varint_s<32>(0x60l);
							}
						} else {
							copy_state3 = (state3 + varint_s<32>(0x60l)) + varint_s<32>(0x0l);
						}
					}
				} else {
					copy_state1 = varint_u<1>(0x0l);
	...
	copy_outputdata_counter1 = state3;
	...
	outputdata_counter1 = copy_outputdata_counter1;
