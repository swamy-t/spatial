...
maxcompilersim::state_machine::Kernel_TopKernel_metapipe1065_sm::Kernel_TopKernel_metapipe1065_sm(maxcompilersim::DebugStreams *debug_output, int node_number) :
...
void
maxcompilersim::state_machine::Kernel_TopKernel_metapipe1065_sm::reset() {
	state2 = 0;
	state3 = 0;
	state4 = 0;
	state5 = 0;
...
void
maxcompilersim::state_machine::Kernel_TopKernel_metapipe1065_sm::execute(const bool sm_not_flushing) {
	...
	varint_u<1> copy_outputdata_sm_last(outputdata_sm_last);
			...
			case 2:
				if (state6) {
					...
					if (varint_u<1>(state2 >= (state4 - varint_u<32>(0x1l)))) {
						copy_state1 = varint_u<4>(0x5l);
					} else {
						copy_state1 = varint_u<4>(0x3l);
					}
				}
				break;
			case 3:
				if (state6 & state7) {
					...
					copy_state2 = state2 + varint_u<32>(0x1l);
					if (varint_u<1>(state2 >= (state4 - varint_u<32>(0x1l)))) {
						copy_state1 = varint_u<4>(0x6l);
					} else {
						copy_state1 = varint_u<4>(0x4l);
					}
				}
				break;
			case 4:
				if ((state6 & state7) & state8) {
					copy_state6 = varint_u<1>(0x0l);
					copy_state7 = varint_u<1>(0x0l);
					copy_state8 = varint_u<1>(0x0l);
					copy_state2 = state2 + varint_u<32>(0x1l);
	...
	copy_outputdata_sm_last = varint_u<1>(0x0l);
	...
	if (inputdata_sm_en) {
		if (varint_u<1>(state2 >= (state4 - varint_u<32>(0x1l)))) {
			copy_outputdata_sm_last = varint_u<1>(0x1l);
		} else {
			copy_outputdata_sm_last = varint_u<1>(0x0l);
		}
		switch (static_cast<int>(state1)) {
			case 1:
				...
				break;
			case 2:
				copy_outputdata_s0_en = (~(state6 | inputdata_s0_done));
				break;
			case 3:
				copy_outputdata_s0_en = (~(state6 | inputdata_s0_done));
				copy_outputdata_s1_en = (~(state7 | inputdata_s1_done));
				break;
			case 4:
				copy_outputdata_s0_en = (~(state6 | inputdata_s0_done));
				copy_outputdata_s1_en = (~(state7 | inputdata_s1_done));
				copy_outputdata_s2_en = (~(state8 | inputdata_s2_done));
				break;
			case 5:
				copy_outputdata_s1_en = (~(state7 | inputdata_s1_done));
				break;
	...
	outputdata_sm_last = copy_outputdata_sm_last;
