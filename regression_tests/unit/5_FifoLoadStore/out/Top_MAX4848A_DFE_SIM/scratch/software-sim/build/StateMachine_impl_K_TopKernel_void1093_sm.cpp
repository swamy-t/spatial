...
maxcompilersim::state_machine::Kernel_TopKernel_void1093_sm::Kernel_TopKernel_void1093_sm(maxcompilersim::DebugStreams *debug_output, int node_number) :
...
void
maxcompilersim::state_machine::Kernel_TopKernel_void1093_sm::reset() {
	state2 = 0;
	state3 = 0;
	state1 = 0;
}
...
void
maxcompilersim::state_machine::Kernel_TopKernel_void1093_sm::execute(const bool sm_not_flushing) {
	...
	if (inputdata_sm_en) {
		switch (static_cast<int>(state1)) {
			case 1:
				copy_outputdata_s0_en = (~(state2 | inputdata_s0_done));
				copy_outputdata_s1_en = (~(state3 | inputdata_s1_done));
				break;
			case 2:
