...
struct QueuedPacketRawStatus {
	...
	QueuedPacketRawStatus(TCPConnection* connection, int status, uint64_t cycle_created) :
		...
		m_event_new = 1; // currently we only have new events
	}
};
...
public:
	explicit TCPRxOutputRawStatus(TCPSync& tcp, const t_port_number port, int conn_bits);
...
private:
	...
	bool m_total_bytes_sent_changed;
	bool m_total_bytes_rcvd_changed;
...
public:
	...
	int getIfVersion() { return m_if_version; }
