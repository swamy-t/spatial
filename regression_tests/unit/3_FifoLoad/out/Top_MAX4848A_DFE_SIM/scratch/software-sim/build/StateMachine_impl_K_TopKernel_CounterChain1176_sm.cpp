...
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1176_sm::Kernel_TopKernel_CounterChain1176_sm(maxcompilersim::DebugStreams *debug_output, int node_number) :
...
void
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1176_sm::reset() {
	state2 = 0;
	state1 = 0;
}
...
void
maxcompilersim::state_machine::Kernel_TopKernel_CounterChain1176_sm::execute(const bool sm_not_flushing) {
	...
	if (inputdata_reset) {
		...
		switch (static_cast<int>(state1)) {
			case 0:
				if (inputdata_en) {
					if (varint_u<1>((state2 + varint_s<32>(0x0l)) >= (inputdata_max0 - varint_s<32>(0x60l)))) {
