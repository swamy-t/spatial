...
maxcompilersim::state_machine::Kernel_TopKernel_mainSm::Kernel_TopKernel_mainSm(maxcompilersim::DebugStreams *debug_output, int node_number) :
...
void
maxcompilersim::state_machine::Kernel_TopKernel_mainSm::reset() {
	state2 = 0;
	state3 = 0;
	state1 = 0;
}
...
void
maxcompilersim::state_machine::Kernel_TopKernel_mainSm::execute(const bool sm_not_flushing) {
	varint_u<1> varint_sm_not_flushing(sm_not_flushing);
	// make copies of current state
	varint_u<64> copy_state2(state2);
	varint_u<64> copy_state3(state3);
	varint_u<3> copy_state1(state1);
	
	// make copies of outputs
	varint_u<64> copy_outputdata_cycles(outputdata_cycles);
	varint_u<1> copy_outputdata_intr_en(outputdata_intr_en);
	varint_u<1> copy_outputdata_run_en(outputdata_run_en);
	...
	if (inputdata_sm_en) {
		switch (static_cast<int>(state1)) {
			case 0:
				copy_state2 = varint_u<64>(0x0l);
				copy_state3 = varint_u<64>(0x0l);
				copy_state1 = varint_u<3>(0x1l);
				break;
			case 1:
				copy_state2 = state2 + varint_u<64>(0x1l);
				if (inputdata_run_done) {
					copy_state1 = varint_u<3>(0x2l);
				}
				break;
			case 2:
				copy_state3 = state3 + varint_u<64>(0x1l);
				if (varint_u<1>(state3 == varint_u<64>(0x200l))) {
					copy_state1 = varint_u<3>(0x3l);
				}
				break;
			case 3:
				if (inputdata_intr_done) {
					copy_state1 = varint_u<3>(0x4l);
					m_debug_output->write("", m_node_number, (boost::format("[main] Done, design ran for %d cycles\n") % (state2)).str());
				}
				break;
			case 4:
				copy_state1 = varint_u<3>(0x4l);
	...
	// output assignment
	copy_outputdata_run_en = varint_u<1>(0x0l);
	copy_outputdata_intr_en = varint_u<1>(0x0l);
	copy_outputdata_sm_done = varint_u<1>(0x0l);
	copy_outputdata_cycles = state2;
	if (inputdata_sm_en) {
		switch (static_cast<int>(state1)) {
			case 1:
				copy_outputdata_run_en = varint_u<1>(0x1l);
				break;
			case 3:
				copy_outputdata_intr_en = varint_u<1>(0x1l);
				break;
			case 4:
	...
	// update outputs
	outputdata_cycles = copy_outputdata_cycles;
	outputdata_intr_en = copy_outputdata_intr_en;
	outputdata_run_en = copy_outputdata_run_en;
