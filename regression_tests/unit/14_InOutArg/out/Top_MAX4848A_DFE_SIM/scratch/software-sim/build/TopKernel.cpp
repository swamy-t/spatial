...
TopKernel::TopKernel(const std::string &instance_name) : 
  ...
  KernelManagerBlockSync(instance_name, 7, 2, 1, 0, "",1)
...
, c_hw_fix_32_0_sgn_bits_1((HWOffsetFix<32,0,TWOSCOMPLEMENT>(varint_u<32>(0x00000004l))))
...
, m_sm_void130_sm(getDebugStreams(), 10)
, m_sm_void132_sm(getDebugStreams(), 5)
{
...
void TopKernel::resetComputationAfterFlush() {
  ...
  { // Node ID: 5 (NodeStateMachine)
    ...
    m_sm_void132_sm.reset();
  }
  { // Node ID: 10 (NodeStateMachine)

    m_sm_void130_sm.reset();
  }
  { // Node ID: 87 (NodeFIFO)
  ...
  { // Node ID: 2 (NodeInputMappedReg)
    id2out_argin123 = getMappedRegValue<HWOffsetFix<32,0,TWOSCOMPLEMENT> >("argin123");
  }
  { // Node ID: 97 (NodeFIFO)
