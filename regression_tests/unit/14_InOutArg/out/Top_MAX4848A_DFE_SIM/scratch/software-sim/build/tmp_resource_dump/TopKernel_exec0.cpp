...
void TopKernel::execute0() {
  ...
  { // Node ID: 14 (NodeStreamOffset)
    const HWOffsetFix<1,0,UNSIGNED> &id14in_input = id87out_output[getCycle()%2];
  ...
  if ( (getFillLevel() >= (4l)))
  { // Node ID: 5 (NodeStateMachine)
    const HWOffsetFix<1,0,UNSIGNED> &id5in_s0_done = id86out_output;
    const HWOffsetFix<1,0,UNSIGNED> &id5in_sm_en = id1out_run_en;
    const HWOffsetFix<32,0,UNSIGNED> &id5in_sm_numIter = id7out_value;

    m_sm_void132_sm.inputdata_s0_done = id5in_s0_done.getBitString();
    m_sm_void132_sm.inputdata_sm_en = id5in_sm_en.getBitString();
    m_sm_void132_sm.inputdata_sm_numIter = id5in_sm_numIter.getBitString();
    m_sm_void132_sm.execute(true);
    id5out_rst_en = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void132_sm.outputdata_rst_en));
    id5out_s0_en = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void132_sm.outputdata_s0_en));
    id5out_sm_done = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void132_sm.outputdata_sm_done));
  }
  { // Node ID: 13 (NodeConstantRawBits)
  }
  if ( (getFillLevel() >= (4l)))
  { // Node ID: 10 (NodeStateMachine)
    const HWOffsetFix<1,0,UNSIGNED> &id10in_ctr_done = id14out_output;
    const HWOffsetFix<1,0,UNSIGNED> &id10in_rst_done = id85out_output;
    const HWOffsetFix<1,0,UNSIGNED> &id10in_sm_en = id5out_s0_en;
    const HWOffsetFix<32,0,TWOSCOMPLEMENT> &id10in_sm_maxIn_0 = id13out_value;

    m_sm_void130_sm.inputdata_ctr_done = id10in_ctr_done.getBitString();
    m_sm_void130_sm.inputdata_rst_done = id10in_rst_done.getBitString();
    m_sm_void130_sm.inputdata_sm_en = id10in_sm_en.getBitString();
    m_sm_void130_sm.inputdata_sm_maxIn_0 = id10in_sm_maxIn_0.getBitString();
    m_sm_void130_sm.execute(true);
    id10out_ctr_en = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void130_sm.outputdata_ctr_en));
    id10out_ctr_maxOut_0 = (HWOffsetFix<32,0,TWOSCOMPLEMENT>( varint_u<32> (m_sm_void130_sm.outputdata_ctr_maxOut_0) ));
    id10out_rst_en = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void130_sm.outputdata_rst_en));
    id10out_sm_done = (HWOffsetFix<1,0,UNSIGNED>(m_sm_void130_sm.outputdata_sm_done));
  }
  { // Node ID: 87 (NodeFIFO)
    const HWOffsetFix<1,0,UNSIGNED> &id87in_input = id10out_ctr_en;

    id87out_output[(getCycle()+1)%2] = id87in_input;
  }
  { // Node ID: 88 (NodeFIFO)
    const HWOffsetFix<1,0,UNSIGNED> &id88in_input = id10out_rst_en;

    id88out_output[(getCycle()+1)%2] = id88in_input;
  }
  { // Node ID: 89 (NodeFIFO)
    const HWOffsetFix<1,0,UNSIGNED> &id89in_input = id10out_sm_done;
  ...
  HWOffsetFix<1,0,UNSIGNED> id6out_output;

  { // Node ID: 6 (NodeStreamOffset)
    const HWOffsetFix<1,0,UNSIGNED> &id6in_input = id5out_sm_done;

    id6out_output = id6in_input;
  }
  { // Node ID: 92 (NodeFIFO)
    const HWOffsetFix<1,0,UNSIGNED> &id92in_input = id6out_output;
  ...
  { // Node ID: 18 (NodeAdd)
    const HWOffsetFix<32,0,TWOSCOMPLEMENT> &id18in_a = id2out_argin123;
    const HWOffsetFix<32,0,TWOSCOMPLEMENT> &id18in_b = id4out_value;

    id18out_result[(getCycle()+1)%2] = (add_fixed<32,0,TWOSCOMPLEMENT,TONEAR>(id18in_a,id18in_b));
  }
  ...
  { // Node ID: 19 (NodeOutputMappedReg)
    ...
    const HWOffsetFix<32,0,TWOSCOMPLEMENT> &id19in_data = id18out_result[getCycle()%2];

    bool id19x_1;

    (id19x_1) = ((id19in_load.getValueAsBool())&(!(((getFlushLevel())>=(4l))&(isFlushingActive()))));
    if((id19x_1)) {
      setMappedRegValue("argout124", id19in_data);
    }
  }
  { // Node ID: 97 (NodeFIFO)
