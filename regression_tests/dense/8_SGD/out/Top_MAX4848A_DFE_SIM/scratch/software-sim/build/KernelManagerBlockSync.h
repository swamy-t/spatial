...
private:
	...
	private:
		...
		const Input& operator=(const Input& rhs) { assert(false); return rhs; /*not possible*/ }
	...
	public:
		Input(int id, const long& cycle_counter, int latency):
			m_latency           (latency),
			m_id                (id),
		...
		void queueRead(bool willRead) {
			m_willReadQueue[(m_cycle + m_latency - 1) % m_latency] = willRead;
		}
		...
		bool wantToRead() const
		{
			...
			return m_willReadQueue[m_cycle % m_latency];
		}
...
public:
	...
	T getMappedRegValue(const std::string &name) {
		...
		std::cout << "Trace kernel " << getName() << ": The value of mapped reg " << name << " was read (= " << d <<  ")" << std::endl;
#endif
...
#ifdef TRACE_KERNEL_MANAGER_BLOCK
		std::cout << "Trace kernel " << getName() << ": Reading " << data << " on input " << getPullInputName(input_port) << std::endl;
#endif
...
#ifdef TRACE_KERNEL_MANAGER_BLOCK
		std::cout << "Trace kernel " << getName() << ": Outputting " << value << " on output " << getPushOutputName(output_port) << std::endl;
#endif
